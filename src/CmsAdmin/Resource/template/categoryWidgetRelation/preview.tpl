...
<ul class="list ui-sortable" id="widget-list" data-category-id="{$categoryId}">
	...
	<li id="widget-item-{$widgetRelation->id}" class="ui-sortable-handle">
		...
		<div class="operation">
			...
			<a href="{@module=cmsAdmin&controller=categoryWidgetRelation&action=toggle&widgetId={$widgetRelation->getWidgetRecord()->id}&categoryId={$categoryId}&id={$widgetRelation->id}@}" class="button toggle-widget" target="_blank" title="pokaż/ukryj"><i class="icon-eye-{if $widgetRelation->active}open{else}close{/if}"></i></a><br />
