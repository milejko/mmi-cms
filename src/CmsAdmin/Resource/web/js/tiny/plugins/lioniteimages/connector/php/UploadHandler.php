...
class UploadHandler
{
    ...
    protected function validate($uploaded_file, $file, $error, $index) {
        ...
        if (!empty($this -> options['accept_file_types']) && !preg_match($this->options['accept_file_types'], $file->name)) {
    ...
    protected function upcount_name($name,$i) {
		if(stripos($name,'-v' . ($i - 1) . '.') !== false) {
			return str_replace('-v' . ($i - 1) . '.','-v' . $i . '.',$name);
		}
		$parts = explode(".", $name);
        $ext = end($parts);
		$base = str_replace(('.'.$ext),"",$name);
		return $base . '-v1.' . $ext;
    }
    ...
    protected function is_valid_image_file($file_path) {
        if (empty($this -> options['image_file_types']) || !preg_match($this->options['image_file_types'], $file_path)) {
            return false;
        }
        if (function_exists('exif_imagetype')) {
