{foreach $files as $file}
    <li>
        <a class="insert ui-icon ui-icon-circle-plus" {if $file['class']=="video" && $file['poster']}data-poster="{thumbBinary($file['poster'], 'scalecrop', '854x480')}"{/if} data-typ="{$file['class']}" href="{$file['full']}" title="dodaj"></a>
        ...
        {if $file['class']=="video"}<a class="edit b_video ico i_play" data-id="{$file['id']}" href="#odtworz"></a><div class="thumbbox video"><video src="{$file['full']}" {if $file['poster']}poster="{thumbBinary($file['poster'], 'scalecrop', '854x480')}"{/if}></video></div>{/if}
    </li>	
{/foreach}